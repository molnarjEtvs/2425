...
with open("lakaseredmenyek.txt","w",encoding="utf-8") as f2:
    for lakas in lakasok:
        f2.write(f"Cím: {lakas.cim}.\n")
        f2.write(f"Terület: {lakas.terulet}m2\n")
        f2.write(f"Érték: {lakas.ertek} Ft\n")
        f2.write(f"Hőmérsélet: {round(lakas.hom,2)}C\n")
        f2.write(f"#"*40)
        f2.write("\n")
